...
export async function scrapeHowardCounty(): Promise<ScraperResult> {
  ...
  try {
    ...
    // Transform permits - include ALL since they're already filtered by B2 prefix
...
async function extractPermitDetails(page: Page, permitNumber: string): Promise<PermitData> {
  ...
  try {
    ...
    console.log(`[howard_county_md] Page text length: ${pageText.length}`);
    ...
    if (workLocationMatch) {
      // Clean up the address - get just the address line
      const rawAddress = workLocationMatch[1];
      // Extract the actual address (typically format: "123 Street Name, City, MD 12345")
    ...
    // Fallback: Look for any address pattern with MD zip code
    ...
    if (descMatch) {
      ...
      // Filter out junk
      if (rawDesc.length > 10 && !rawDesc.toLowerCase().includes('spell check') && !rawDesc.includes('function')) {
    ...
    console.log(`[howard_county_md] Extracted - Address: "${permitData.address}", Description: "${permitData.description?.substring(0, 50)}..."`);

  } catch (error) {
    console.error(`[howard_county_md] Error extracting permit details:`, error);
  }
