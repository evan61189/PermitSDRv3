...
async function extractPermitDetails(page: Page, permitNumber: string): Promise<PermitData> {
  ...
  const isValidAddress = (text: string): boolean => {
    ...
    // Should contain numbers and letters typical of addresses
    const hasNumber = /\d/.test(text);
    const hasLetter = /[a-zA-Z]/.test(text);
    // Common address words
    const addressWords = ['st', 'street', 'ave', 'avenue', 'rd', 'road', 'dr', 'drive', 'blvd', 'ln', 'lane', 'way', 'ct', 'court', 'pl', 'place', 'baltimore', 'md'];
    const lowerText = text.toLowerCase();
          ...
          if (await labelEl.isVisible({ timeout: 500 })) {
            ...
            if (await parent.isVisible({ timeout: 300 })) {
              ...
              for (const cell of cells) {
                ...
                // Validate it's a real address, not JavaScript
