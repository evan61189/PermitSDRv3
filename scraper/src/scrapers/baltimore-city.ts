...
async function extractPermitDetails(page: Page, permitNumber: string): Promise<PermitData> {
  ...
  try {
    ...
    if (!permitData.address) {
      try {
        // Look for a section/div containing "Work Location" header
    ...
    if (!permitData.address) {
      try {
        ...
        for (const span of spans) {
          ...
          // Only check spans that seem address-related by ID
    ...
    // Method 1: Try finding by ID patterns for Project Description (most reliable for Baltimore City)
    const descSelectors = [
    ...
    // Method 2: Look for Project Description section and get text after the label
    if (!permitData.description) {
      try {
        // Find all table rows and look for ones with Project Description label
        const rows = await page.locator('tr').all();
        for (const row of rows) {
          const rowText = await row.textContent() || '';
          const rowLower = rowText.toLowerCase();

          // Check if this row contains a description label
          if (rowLower.includes('project description') || rowLower.includes('description of work')) {
            // Get all spans in this row that might contain the actual description
    ...
    if (!permitData.description) {
      try {
        ...
        for (const span of spans) {
          ...
          // Look for description fields, exclude licensed professional
